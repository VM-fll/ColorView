...
def get_average_hsv(hsv_image, center, radius):
    x, y = center
    samples = []
    for dx in range(-1, 2):
        for dy in range(-1, 2):
...
def get_hsv_values(image_path):
    ...
    if circles is not None:
        ...
        for i, (x, y, r) in enumerate(circles):
            ...
            hsv_value = get_average_hsv(hsv_image, (x, y), r)
...
def show_camera_feed(label):
    ...
    label.img_tk = img_tk
    label.config(image=img_tk)
    label.after(10, show_camera_feed, label)
...
def create_button(canvas, image, command):
    button_id = canvas.create_image(0, 0, anchor='nw', image=image)
    canvas.tag_bind(button_id, "<Button-1>", lambda e: command())

# Title label as an image
title_canvas = Canvas(home_frame, width=title_image.width(), height=title_image.height(), highlightthickness=0)
title_canvas.pack(pady=20)
title_canvas.create_image(0, 0, anchor='nw', image=title_image)

# Start button
start_button_canvas = Canvas(home_frame, width=start_button_image.width(), height=start_button_image.height(), highlightthickness=0)
start_button_canvas.pack(pady=20)
create_button(start_button_canvas, start_button_image, start_detection)

# Capture button
capture_button_canvas = Canvas(home_frame, width=capture_button_image.width(), height=capture_button_image.height(), highlightthickness=0)
capture_button_canvas.pack(pady=20)
create_button(capture_button_canvas, capture_button_image, capture_image)

# Frame to show camera feed while capturing
capture_frame = Frame(root)
capture_label = Label(capture_frame)
capture_label.pack()

# Save button
save_button_canvas = Canvas(capture_frame, width=save_button_image.width(), height=save_button_image.height(), highlightthickness=0)
save_button_canvas.pack(pady=20)
create_button(save_button_canvas, save_button_image, save_captured_image)

# Calibrate camera button
calibrate_button_canvas = Canvas(home_frame, width=calibrate_button_image.width(), height=calibrate_button_image.height(), highlightthickness=0)
calibrate_button_canvas.pack(pady=20)
create_button(calibrate_button_canvas, calibrate_button_image, calibrate_camera)
